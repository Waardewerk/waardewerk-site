...
export interface NewsItem {
  slug: string;
  title: string;
  date: string;           // ISO YYYY-MM-DD, bepaalt sortering
  dateLabel?: string;     // optionele weergavetekst i.p.v. de volledige datum, bv. 'Juni 2026'
  ...
  spotify?: string;       // Spotify episode-ID voor embed
}

export const NEWS: NewsItem[] = [
  {
    slug: 'praktijkmeesters-podcast-patrick-wagenaar',
    title: 'Te gast in de Praktijkmeesters podcast van Patrick Wagenaar',
    date: '2026-06-15',
    dateLabel: 'Juni 2026',
    tag: 'Podcast',
    ...
    content: [
      `"Dat lossen we wel op met meer mensen." Alleen zijn die mensen er niet. En dat wordt alleen maar nijpender.`,
      `In aflevering #140 sprak Patrick Wagenaar met Ruud Blom over wat er gebeurt als het werk blijft komen — en de vakman verdwijnt. In het straatwerk stromen jaarlijks meer mensen uit dan er bijkomen. Niet een beetje. Structureel.`,
      `Doordat je de mensen niet meer vindt, is de echte vraag hoe je het werk anders organiseert.`,
      `Over robotisering, personeelstekorten, sociale innovatie — en waarom verandering geen keuze meer is, maar een voorwaarde om te kunnen blijven bestaan.`,
    ],
  },
  {
...
export function getDateLabel(item: NewsItem): string {
  return item.dateLabel ?? formatNewsDate(item.date);
}
